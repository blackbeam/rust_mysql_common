...
pub struct DeleteRowsEvent<'a>(RowsEvent<'a>);

impl<'a> DeleteRowsEvent<'a> {
    ...
    pub fn into_owned(self) -> DeleteRowsEvent<'static> {
        DeleteRowsEvent(self.0.into_owned())
    }
}
...
impl<'a> BinlogStruct<'a> for DeleteRowsEvent<'a> {
